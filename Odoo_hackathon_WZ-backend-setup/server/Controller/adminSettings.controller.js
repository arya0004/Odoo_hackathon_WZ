...
// ✅ Update salary info
import { SalaryStructure } from "../config/db.js";
  ...
  try {
    ...
    if (salary_structure_id) {
      const st = await SalaryStructure.findOne({
        where: { salary_structure_id, company_id: req.user.company_id },
      });
      if (!st) return res.status(400).json({ message: "Invalid salary_structure_id for your company" });
    }

    const [updated] = await User.update(req.body, {
      where: { user_id, company_id: req.user.company_id },
    });

    if (!updated) return res.status(404).json({ message: "Employee not found in your company" });
