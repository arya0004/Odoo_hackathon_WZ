...
export const getStatus = async (req, res) => {
  try {
    ...
    // If checked out already -> Absent (workday finished)
    if (record.check_out) return res.json({ status: "Absent" });

    // Checked in and not checked out -> Present
    if (record.check_in && !record.check_out) return res.json({ status: "Present" });
