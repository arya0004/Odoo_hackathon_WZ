...
// /* remove empty strings/undefined from payload */
// function clean(obj) {
//   const out = {};
...
//   // profile + salary slices
//   const [profile, setProfile] = useState(null);          // identity + private/bank
//   const [salaryInfo, setSalaryInfo] = useState(null);    // for Admin/Payroll
...
//         if (!viewingOther) {
//           // SELF
//           const r = await api("/employee/myprofile"); // ← unified endpoint (no hyphen)
//           setProfile(r.profile);
...
//       setSaving(true);
//       if (viewingOther && me.role === "Admin") {
//         await api(`/admin/employee-profile/${targetId}`, { method: "PUT", body: JSON.stringify(clean(edit)) });
//       } else {
//         // unify to /employee/myprofile (no hyphen)
//         await api("/employee/myprofile", { method: "PUT", body: JSON.stringify(clean(edit)) });
//       }
//       showToast("Profile updated ✅");
//     } catch (e) {
//       showToast(e.message, "error");
...
//     try {
//       setSaving(true);
//       await api(`/admin/salary-info/${targetId}`, { method: "PUT", body: JSON.stringify(clean(salaryInfo)) });
//       showToast("Salary info saved 💾");
//     } catch (e) {
//       showToast(e.message, "error");
//     } finally {
...
//                   <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
//                     <Card title={`Salary Info ${perms.canEditSalary ? "" : "(read-only)"}`}>
//                       <MoneyRow label="Monthly Wage" value={salaryInfo?.monthly_wage}
//                         onChange={(v) => setSalaryInfo((s) => ({ ...(s || {}), monthly_wage: v }))} editable={perms.canEditSalary} />
//                       <MoneyRow label="Basic Salary" value={salaryInfo?.basic_salary}
//                         onChange={(v) => setSalaryInfo((s) => ({ ...(s || {}), basic_salary: v }))} editable={perms.canEditSalary} />
//                       <MoneyRow label="House Rent Allowance" value={salaryInfo?.hra}
//                         onChange={(v) => setSalaryInfo((s) => ({ ...(s || {}), hra: v }))} editable={perms.canEditSalary} />
//                       <MoneyRow label="Standard Allowance" value={salaryInfo?.standard_allowance}
//                         onChange={(v) => setSalaryInfo((s) => ({ ...(s || {}), standard_allowance: v }))} editable={perms.canEditSalary} />
//                       <MoneyRow label="Performance Bonus" value={salaryInfo?.performance_bonus}
//                         onChange={(v) => setSalaryInfo((s) => ({ ...(s || {}), performance_bonus: v }))} editable={perms.canEditSalary} />
//                       <MoneyRow label="Leave Travel Allowance" value={salaryInfo?.leave_travel_allowance}
//                         onChange={(v) => setSalaryInfo((s) => ({ ...(s || {}), leave_travel_allowance: v }))} editable={perms.canEditSalary} />
//                       <MoneyRow label="Fixed Allowance" value={salaryInfo?.fixed_allowance}
//                         onChange={(v) => setSalaryInfo((s) => ({ ...(s || {}), fixed_allowance: v }))} editable={perms.canEditSalary} />
...
/* ==== Self-profile route guards (tries both spellings) ==== */
async function getSelfProfile() {
  try {
    return await api("/employee/my-profile"); // preferred
  } catch (e) {
    if (e.status === 404) return await api("/employee/myprofile"); // fallback
    throw e;
  }
}
async function putSelfProfile(body) {
  try {
    return await api("/employee/my-profile", { method: "PUT", body: JSON.stringify(body) });
  } catch (e) {
    if (e.status === 404) {
      return await api("/employee/myprofile", { method: "PUT", body: JSON.stringify(body) });
    }
    throw e;
  }
}
...
export default function MyProfile() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        if (!viewingOther) {
          // SELF
          const r = await getSelfProfile(); // ← route-agnostic
          setProfile(r.profile);
  ...
  const savePrivate = async () => {
    try {
      ...
      if (viewingOther && me.role === "Admin") {
        ...
        await putSelfProfile(clean(edit)); // ← route-agnostic
      }
      showToast("Profile updated ✅");
    } catch (e) {
      showToast(e.message, "error");
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {toast && (
        <div
          className={`fixed bottom-6 left-1/2 -translate-x-1/2 px-4 py-2 rounded-lg shadow border ${toast.type === "error" ? "bg-rose-50 border-rose-200 text-rose-700" : "bg-emerald-50 border-emerald-200 text-emerald-700"
            }`}
        >
          {toast.msg}
        </div>
